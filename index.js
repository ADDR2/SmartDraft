...
function evaluateHeroe(heroe = {}, allyPicks, enemyPicks, mapId) {
  ...
  sum += allyPicks.reduce((total, pickId) => {
    const goodSinergyAmount =
      pickId in heroe.goodSinergy && !(pickId in heroe.badSinergy)
        ? heroe.goodSinergy[pickId].amount
        : 0;
    const badSinergyAmount =
      pickId in heroe.badSinergy && !(pickId in heroe.goodSinergy)
        ? heroe.badSinergy[pickId].amount
        : 0;
  ...
  sum += enemyPicks.reduce((total, pickId) => {
    const goodAgainstAmount =
      pickId in heroe.goodAgainst && !(pickId in heroe.badAgainst)
        ? heroe.goodAgainst[pickId].amount
        : 0;
    const badAgainstAmount =
      pickId in heroe.badAgainst && !(pickId in heroe.goodAgainst)
        ? heroe.badAgainst[pickId].amount
        : 0;
  ...
  const goodMapAmount =
    mapId in heroe.goodIn && !(mapId in heroe.badIn)
      ? heroe.goodIn[mapId].amount
      : 0;
  const badMapAmount =
    mapId in heroe.badIn && !(mapId in heroe.goodIn)
      ? heroe.badIn[mapId].amount
      : 0;
...
function search(query) {
  const { allyPicks = [], enemyPicks = [], mapId = '' } = query;
  const result = [];

  for (const heroe of knowledge) {
    isValid(heroe, query) &&
      result.push(evaluateHeroe(heroe, allyPicks, enemyPicks, mapId));
  }
